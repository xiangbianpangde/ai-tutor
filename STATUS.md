...
10. ~~**#21 用户旅程**~~ ✅ 2026-06-15：整条旅程真驱动跑通（含**零基础一键导入建科目**）——
    安装 `AI-Tutor Setup 2.0.0.exe`（已真装到 AppData/Local/Programs/AI-Tutor）→ 向导 →
    学习中心 → **导入资料新建科目**（概率论 markdown→6 概念）或选现成科目 → 开会话 → 作答 →
    判分反馈 → WebSocket 实时。可用性评估 `doc/reports/21-终极验收-可用性评估.md` + 6 张 journey-*.png。
11. **唯一剩余 = 人本内核**：让某个真人主观体验打分"好不好用/教得透不透"（AI 无法替代）；
    可选工程项：代码签名证书、真向量后端/NLI（规模化）、真 LLM key 判分主路径、D2/D3 人签核。
