...
> 更新: 2026-06-13（C2 进行中：M-005/M-004/M-007/M-006/M-017 五块就位，848 测试全绿，仅余 M-014 数据管线 / v2 第一波过堂三机制结构性通过 / C1+D1 已签核）
> 流程档位: 标准
...
- ◐ **C2 第二波**（进行中）M-004 Session + M-005 Cache + M-006 事件总线 + M-007 Pipeline + M-014 数据管线（对应 v2 #3+#6+#7+#8）
  ...
  - ✅ **M-017 FileManager**（`{user}/{subject}/{kind}/` 人类可读目录替 UUID + 路径穿越防护，848 测试全绿）
    → `worklogs/2026-06-13_C2-M017-文件管理器.md`
  - ☐ **M-014 数据管线**（C2 最后一块，最大：内容级抽取/噪声<5% 红线 + 换体裁回灌）
...
4. **C2 第二波**（进行中）：✅ 文档去 sys.path + ✅ M-005 + ✅ M-004 + ✅ M-007 + ✅ M-006 + ✅ M-017。
   **下一步** = **M-014 数据管线**（C2 最后一块、最大：采集→清洗→建图 + 内容级抽取/噪声<5% 红线 + 换体裁回灌）→ C2 收尾。
