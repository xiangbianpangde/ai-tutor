...
_（从 ROADMAP.md 和未完成的 spec 中提取，列出最优先的 1-3 项）_

**当前阶段**：计划已完成（v2 11 功能点 + v3.1 17 模块细化升级），**等用户 review 后进入开发**。

最优先 1-3 项（按 v3.1↔v2 桥接文档的"五波推进"建议）：

1. **Review v3.1 设计包**（最高优先 · 阻塞开发启动）：按 `doc/plan/v3.1-aitutor-design/REVIEW-CHECKLIST.md` 7 段 60min 路线走完，决定 v3.1 走向（A/B/C/D 四选一，推荐 C）
2. **启动 v2 #1 后端骨架 + v3.1 M-001 服务入口 + M-002 API 网关**：第一波基础，对应 v2 的 1 个 BDD + v3.1 的 2 个模块文件框架（`06-framework/M-001/` 和 `M-002/`）
3. **完善 5 个 🟡 策略状态机**（v2 #10 部分 + v3.1 M-010 费曼评分）：feynman/socratic/pbl/analogy/spaced_repetition 从精简状态机升级到完整实现
