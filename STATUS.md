...
- [ ] **换体裁攻击**：选一个**非博客体裁**新科目（数学类 PDF / 英文官方文档 / 视频转写
      ...
      换体裁大概率漏新形态——全量禁类 <5% 才算过；超了就补规则+回归测试（学我），
      或老实承认该形态属 v2 M-014。**不许拿 FastAPI 这个已调优科目反复演示充数。**
...
- [ ] **判分反向攻击（我没测完的半边）**：3 道题各给"一句话但切中要害"的极简正确答案
      ——不得因为短就判 partial/incorrect（防判分器奖励长篇大论）。FIX-D 只验证了
      "长而不答题必死"；"短而答对必活"留给你，挂了就修 llm_scorer 并留测试。
- [ ] **重建幂等回归**：对 fastapi-houduankaifa **重新 acquire**（会生成新 corpus）+
      build(full)——必须零手工删库成功（K3 回归）。⚠️ 已知暗坑：新 corpus 的概念 id
      章节号可能漂移 → 旧 BKT 进度孤儿化。v1 不要求迁移，但 worklog 里必须写清
      进度丢没丢、丢了多少——**瞒着不写算挂**。
- [ ] **不许删测试刷绿**：FIX-K 的 9 条回归（test_kg_toc / test_research_adapter /
      test_study_pack）一条不删、断言不弱化；792 基线只许涨不许跌。
