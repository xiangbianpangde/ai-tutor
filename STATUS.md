...
- [ ] **v3.1 review 不许盖章式通过**：走向决议 worklog 必须包含——①5 个关键风险
      （R-16 / R1 / C2 / pdf2zh AGPL / sqlite-vec）逐个一句处置；②A/B/C/D 给出
      **否决其余三项**的理由（只说"选 C"不算）；③对 `V3.1-vs-V2-桥接.md` §六做
      交叉引用核对并列出冲突/重复清单（阶段 A4）。
- [ ] **换体裁攻击**：选一个**非博客体裁**新科目（数学类 PDF / 英文官方文档 / 视频转写
      任一）跑 acquire→build→review。噪声过滤是在 FastAPI 博客语料上六轮调出来的，
      换体裁大概率漏新形态。过堂方式二选一，都算过：
      ① 全量禁类 <5% 且补了新形态规则 + 回归测试；
      ② 全量扫描后如实贴出超标数字 + 把新形态归类回灌 `doc/plan/开发清单.md` M-014 验收红线。
      唯一不算过的：拿 FastAPI 这个已调优科目演示充数，或只贴抽样不贴全量。
- [ ] **全量扫描，不是抽样**：声称噪声达标必须贴**全量概念名扫描**数字。教训：第 4 轮
      30 抽样"看着过了"，第 5 轮就抽出漏网的路径类（app/main.py）——30 抽样过 ≠ 过。
- [ ] **判分反向攻击（我没测完的半边）**：3 道题各给"一句话但切中要害"的极简正确答案，
      检验判分器是否因为短就压成 partial/incorrect（防奖励长篇大论）。
      **发现它挂 = 得分**（修 llm_scorer + 留回归测试即过堂）；直接全过也过堂（贴判分记录）。
      这题不存在"做了但挂"的结局，只存在"没做"。
- [ ] **重建幂等回归**：用**牺牲品科目**（冒烟科目 climaoyanceshi，或上面换体裁的新科目）
      同名重新 acquire（生成新 corpus）+ build(full)——必须零手工删库成功（K3 回归）。
      ⚠️ 已知暗坑（明牌告知）：新 corpus 概念 id 章节号可能漂移 → 旧 BKT 进度孤儿化。
      v1 不要求迁移，但 worklog 必须写清进度丢没丢、丢了多少——如实披露即过堂。
      **再说一遍：不许碰 fastapi-houduankaifa。**
- [ ] **不许删测试刷绿**：FIX-K 的 9 条回归（test_kg_toc / test_research_adapter /
      test_study_pack）一条不删、断言不弱化；v1 范围内 792 基线只许涨不许跌
      （新挑战发现 bug 导致先红后绿属正常，红→绿过程写进 worklog 即可）。
      `git diff HEAD -- '*test*'` 自证清白。

> 当前目标：**做 v3.1 review + 走向决议（上面第 1 项）**——这是 v2 动工的唯一阻塞项。
> 复跑验证已完成（2026-06-13），FastAPI 演示科目就绪（tutor_cli learn 即可继续教学）。
> 接手者请先读 `worklogs/2026-06-12_复跑验证-tutor_cli-FIX-K.md`，再过第二批挑战。
