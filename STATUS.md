...
> 更新: 2026-06-13（C2 进行中：M-005 缓存 + M-004 会话 + M-007 异步任务 + M-006 事件总线，837 测试全绿 / v2 第一波过堂三机制结构性通过 / C1+D1 已签核）
> 流程档位: 标准
...
- ◐ **C2 第二波**（进行中）M-004 Session + M-005 Cache + M-006 事件总线 + M-007 Pipeline + M-014 数据管线（对应 v2 #3+#6+#7+#8）
  ...
  - ✅ **M-006 EventBus**（进程内 pub/sub + EventRecorder + `GET /api/meta/events`，#7推送/#11监控地基，837 测试全绿）
    → `worklogs/2026-06-13_C2-M006-事件总线.md`
  - ☐ M-017 FileManager（人类可读目录替 UUID）/ M-014 数据管线（内容级抽取/噪声<5%）
...
4. **C2 第二波**（进行中）：✅ 文档去 sys.path + ✅ M-005 Cache + ✅ M-004 Session + ✅ M-007 异步任务 + ✅ M-006 EventBus。
   **下一步** = M-017 FileManager（人类可读目录替 UUID）→ M-014 数据管线（C2 最大一块，含内容级抽取/噪声<5% 红线）。
